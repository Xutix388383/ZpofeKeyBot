...
def get_time_since_created(created_at_iso):
    ...
    async def view_all_tickets(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if tickets:
            for ticket in tickets[-10:]:  # Show last 10 tickets
                status_emoji = "✅" if ticket['status'] == 'verified' else "⏳"
                time_open = get_time_since_created(ticket['created_at'])

                # Get order details
                order = next((o for o in orders if o['id'] == ticket['order_id']), None)
                scripts_info = "Unknown"
                if order and order.get('items'):
                    script_names = [item['name'] for item in order['items']]
                    scripts_info = ", ".join(script_names)
...
class ShopView(discord.ui.View):
    ...
    async def checkout(self, interaction: discord.Interaction):
        ...
        await self.process_checkout(interaction, self.cart)

    async def process_checkout(self, interaction: discord.Interaction, cart):
        orders = load_orders()
        ...
        order = {
            'id': len(orders) + 1,
            'items': cart,
            'buyer_email': "N/A",  # Email is not collected directly
            'buyer_discord': interaction.user.name,  # Discord username from interaction
        ...
        guild = interaction.guild
        category = None  # You may want to specify a category for ticket channels
        ...
        try:
            ...
            overwrites = {
                ...
                guild.get_member(1385239185006268457): discord.PermissionOverwrite(read_messages=True, send_messages=True) if guild.get_member(1385239185006268457) else None
            }
            ...
            # Calculate time since ticket opened
            created_time = datetime.fromisoformat(ticket['created_at'])
            time_opened = "Just opened"

            # Get script names
            script_names = [item['name'] for item in cart]
            scripts_text = "\n".join([f"• {name}" for name in script_names])

            # Create ticket embed
            ticket_embed = discord.Embed(
                title="🎫 New Purchase Ticket",
                description=f"<@1385239185006268457> New purchase from {interaction.user.mention}",
                color=0x667eea,
                timestamp=datetime.now()
            )
            ...
            ticket_embed.add_field(
                ...
                value=f"**Time Opened:** {time_opened}\n**Order ID:** #{order['id']}\n**Ticket ID:** #{ticket['id']}",
                inline=False
            )

            ticket_embed.add_field(
                name="🛒 Scripts Purchased",
                value=scripts_text,
                inline=False
            )
            ...
            ticket_embed.add_field(
                ...
                value=f"**Total:** ${total_price:.2f}\n**Payment Method:** Cash App\n**Cash App:** https://cash.app/$EthanCreel1",
                inline=False
            )

            ticket_embed.add_field(
                name="📋 Payment Instructions",
                value="**How to Pay:**\n1. Send payment to: https://cash.app/$EthanCreel1\n2. Take a screenshot of the payment confirmation\n3. Upload the screenshot in this ticket\n4. Wait for Zpofe to verify payment and assign scripts to your user ID\n5. You'll automatically receive the buyer role\n6. **IMPORTANT:** Go to the panel channel and click 'Get My Scripts' to download (buyer role required)",
                inline=False
            )
            ...
            ticket_embed.set_footer(text="Zpofe will assign your scripts shortly after payment verification")

            # Send message in ticket channel
            await ticket_channel.send(
                content=f"<@1385239185006268457> {interaction.user.mention}",
                embed=ticket_embed
            )
            ...
            embed = discord.Embed(
                title="✅ Purchase Complete - Ticket Created!",
                description=f"**Script will be assigned to you after purchase!**\n\nYour ticket: {ticket_channel.mention}",
                color=0x28a745
            )
            ...
            embed.add_field(
                name="🎯 Next Steps",
                value="1. **Go to your ticket** and complete payment\n2. **Wait for Zpofe** to assign scripts to your user ID\n3. **Get buyer role** automatically after verification\n4. **Go to panel channel** and hit 'Get My Scripts' button\n5. **Download your scripts** after receiving buyer role!",
                inline=False
            )

            embed.add_field(
                name="⚠️ Important",
                value=f"You **MUST have the buyer role** before clicking 'Get My Scripts' in the panel channel.\n\nTicket: {ticket_channel.mention}",
                inline=False
            )

            embed.set_footer(text="Scripts assigned after payment verification | Must have buyer role to access")
            ...
            embed = discord.Embed(
                title="✅ Order Complete - Script Will Be Assigned!",
                description=f"**Script will be assigned to you after purchase!**",
                color=0x28a745
            )
            ...
            embed.add_field(
                name="🎯 Next Steps",
                value="1. **Send payment** to: https://cash.app/$EthanCreel1\n2. **Contact** <@1385239185006268457> with payment proof\n3. **Wait for Zpofe** to assign scripts to your user ID\n4. **Get buyer role** automatically after verification\n5. **Go to panel channel** and click 'Get My Scripts' button (must have buyer role)",
                inline=False
            )
...
class CartView(discord.ui.View):
    ...
    async def proceed_checkout(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Direct checkout without form
        await self.shop_view.process_checkout(interaction, self.cart)
...
async def get_scripts_panel(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="🎫 Requirements",
        value="• Must have the Buyer role (assigned after payment verification)\n• Must have purchased and paid for scripts\n• Scripts will be assigned to your user ID after payment\n• Scripts will be sent via DM when you click the button",
        inline=False
    )
...
class GetScriptsPanelView(discord.ui.View):
    ...
    async def get_my_scripts(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Send user's scripts via DM - buyer role required"""
        if not has_buyer_role(interaction.user):
            await interaction.response.send_message("❌ You need the Buyer role to access your scripts!\n\n**How to get the Buyer role:**\n1. Purchase scripts through the shop\n2. Complete payment in your ticket\n3. Wait for Zpofe to verify and assign scripts to your user ID\n4. Return here after verification to download your scripts", ephemeral=True)
            return
...
async def get_scripts(interaction: discord.Interaction):
    """Allow buyers to get their scripts"""
    if not has_buyer_role(interaction.user):
        await interaction.response.send_message("❌ You need the Buyer role to access your scripts!\n\n**How to get the Buyer role:**\n1. Purchase scripts through the shop\n2. Complete payment in your ticket\n3. Wait for Zpofe to assign scripts to your user ID\n4. Use this command after verification to access your scripts", ephemeral=True)
        return
